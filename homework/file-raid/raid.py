...
class raid:
    def __init__(self, chunkSize='4k', numDisks=4, level=0, timing=False, reverse=False, solve=False, raid5type='LS'):
        ...
        if (chunkSize % BLOCKSIZE) != 0:
            print('chunksize (%d) must be multiple of blocksize (%d): %d' %
    ...
    def stats(self, totalTime):
        for d in range(self.numDisks):
            ...
            if totalTime > 0.0:
                ...
                util = 0.0
            if s[4] == totalTime:
                print('disk:%d  busy: %.2f  I/Os: %5d (sequential:%d nearly:%d random:%d)' %
    ...
    def enqueue(self, addr, size, isWrite):
        ...
        if self.timing == False:
            if self.solve or self.reverse == False:
                if isWrite:
                    print('LOGICAL WRITE to  addr:%d size:%d' %
                          (addr, size * BLOCKSIZE))
                else:
                    print('LOGICAL READ from addr:%d size:%d' %
                          (addr, size * BLOCKSIZE))
                if self.solve == False:
    ...
    def doPartialWrite(self, stripe, begin, end, bmap, pmap):
        ...
        if (numWrites + 1) <= (self.blocksInStripe - numWrites):
            ...
            for voff in range(stripeBegin, begin):
                (disk, off) = bmap(voff)
                self.doSingleRead(disk, off, (voff == (
                    begin - 1)) and (end == stripeEnd))
    ...
    def enqueue45(self, addr, size, isWrite):
        ...
        if isWrite == False:
            ...
            for stripe in range(initStripe, finalStripe + 1):
                endOfStripe = (stripe * self.blocksInStripe) + \
                    self.blocksInStripe
...
parser.add_option('-D', '--numDisks',    default=4,      help='number of disks in RAID',
                  action='store',       type='int',    dest='numDisks')
parser.add_option('-C', '--chunkSize',   default='4k',   help='chunk size of the RAID',
                  action='store',       type='string', dest='chunkSize')
parser.add_option('-n', '--numRequests', default=10,     help='number of requests to simulate',
                  action='store',       type='int',    dest='numRequests')
parser.add_option('-S', '--reqSize',     default='4k',   help='size of requests',
                  action='store',       type='string', dest='size')
parser.add_option('-W', '--workload',    default='rand', help='either "rand" or "seq" workloads',
                  action='store',       type='string', dest='workload')
parser.add_option('-w', '--writeFrac',   default=0,      help='write fraction (100->all writes, 0->all reads)',
                  action='store',       type='int',    dest='writeFrac')
parser.add_option('-R', '--randRange',   default=10000,  help='range of requests (when using "rand" workload)',
                  action='store',       type='int',    dest='range')
parser.add_option('-L', '--level',       default=0,      help='RAID level (0, 1, 4, 5)',
                  action='store',       type='int',    dest='level')
parser.add_option('-5', '--raid5',       default='LS',   help='RAID-5 left-symmetric "LS" or left-asym "LA"',
...
parser.add_option('-t', '--timing',      default=False,  help='use timing mode, instead of mapping mode',
...
if size % BLOCKSIZE != 0:
    print('error: request size (%d) must be a multiple of BLOCKSIZE (%d)' %
          (size, BLOCKSIZE))
    exit(1)
...
assert (options.level == 0 or options.level ==
        1 or options.level == 4 or options.level == 5)
