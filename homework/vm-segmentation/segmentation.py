...
parser.add_option("-a", "--asize", default="1k", help="address space size (e.g., 16, 64k, 32m, 1g)",
...
parser.add_option("-b", "--b0", default="-1", help="value of segment 0 base register",
                  action="store", type="string", dest="base0")
parser.add_option("-l", "--l0", default="-1", help="value of segment 0 limit register",
                  action="store", type="string", dest="len0")
parser.add_option("-B", "--b1", default="-1", help="value of segment 1 base register",
                  action="store", type="string", dest="base1")
parser.add_option("-L", "--l1", default="-1", help="value of segment 1 limit register",
...
abort_if(psize <= asize,
         'physical memory size > address space size (for this simulation)')
...
print('')
print('  Segment 1 base  (grows negative) : 0x%08x (decimal %d)' %
...
abort_if((len0 + base0) > base1 and (base1 > base0),
...
for vstr in addrList:
    ...
    if vaddr < 0 or vaddr >= asize:
        print('Error: virtual address %d cannot be generated in an address space of size %d' % (
            vaddr, asize))
        exit(1)
    if options.solve == False:
        ...
        if (vaddr >= (asize / 2)):
            ...
            if paddr < base1:
                print(
                    '  VA %2d: 0x%08x (decimal: %4d) --> SEGMENTATION VIOLATION (SEG1)' % (i, vaddr, vaddr))
            else:
            ...
            if (vaddr >= len0):
                print(
                    '  VA %2d: 0x%08x (decimal: %4d) --> SEGMENTATION VIOLATION (SEG0)' % (i, vaddr, vaddr))
            else:
                paddr = vaddr + base0
